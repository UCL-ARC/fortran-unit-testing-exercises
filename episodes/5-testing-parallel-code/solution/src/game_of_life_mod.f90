...
contains
    ...
    subroutine evolve_board(current_board, new_board)
        ...
        integer :: row, col, num_rows, num_cols, sum, num_threads, thread_id

        num_rows = size(current_board, 1)
        num_cols = size(current_board, 2)

        !$omp parallel do default(none) private(row, col, sum) shared(num_rows, num_cols, current_board, new_board)
        do row=2, num_rows-1
            do col=2, num_cols-1
